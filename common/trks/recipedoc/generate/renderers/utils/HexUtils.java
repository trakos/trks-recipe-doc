/**
 *
 * Comes from org.apache.kahadb.util
 *
...
public final class HexUtils
{
    ...
    public static String toHexFromBytes(byte[] bytes)
    {
        ...
        {
            stringBuilder.append(HEX_TABLE[0xFF & aByte]);
        }
    ...
    {
        StringBuilder stringBuilder = new StringBuilder(INT_OFFSETS.length * 2);
        for (int INT_OFFSET : INT_OFFSETS)
        {
            int b = 0xFF & (value >> INT_OFFSET);
            if (!(trim && b == 0))
            {
                stringBuilder.append(HEX_TABLE[b]);
                trim = false;
            }
        }
        return stringBuilder.toString();
